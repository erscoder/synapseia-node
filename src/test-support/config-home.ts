/**
 * Side-effect setup for config.spec.ts.
 *
...
export const CONFIG_TEST_HOME = join(
  tmpdir(),
  `synapseia-test-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2)}`,
);

process.env.SYNAPSEIA_HOME = CONFIG_TEST_HOME;
