 ...
 */
export async function promptForPassword(message: string = 'Enter wallet password: '): Promise<string> {
...
export async function generateWallet(
  ...
  if (!password) {
    password = await promptForNewPassword();
  }
 ...
 */
export function getWalletAddress(walletDir: string = WALLET_DIR): string {
  try {
    const walletPath = path.join(walletDir, 'wallet.json');
    if (!existsSync(walletPath)) {
 ...
 */
export function hasWallet(walletDir: string = WALLET_DIR): boolean {
  const walletPath = path.join(walletDir, 'wallet.json');
 ...
 */
export function displayWalletCreationWarning(wallet: SolanaWallet): void {
...
export async function changeWalletPassword(
  ...
  const wallet = await loadWallet(walletDir, oldPassword);
