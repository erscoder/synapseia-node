...
## [0.9.10] 2026-06-01 fix(agent): stop re-iterating cancelled/closed-round work orders (42ce9381)
...
## [2026-05-30] fix(wallet): harden keystore atomic write — enforce 0o600 + unique tmp name (93a9ce65)

Audit remediation (L1/L2/L5) of the new `atomicWriteFileSync` (guards the only copy of the encrypted secret key). L1: `openSync` `mode` applies only on CREATE, so a pre-existing loose-mode `.tmp` would let the final `wallet.json` inherit world/group-readable perms — now `fchmodSync(fd, 0o600)` right after open enforces it unconditionally (mirrors `EncryptedKeystore.save`). L2: the fixed `${target}.tmp` name let two concurrent writers (the batch made `loadWallet` re-encrypt+persist on every unlock; a node-ui bg process + an operator `syn` command can both reach it) truncate/cross-delete each other's tmp -> spurious "upgrade failed" warn + a lost (self-healing) upgrade — now a unique `${target}.${pid}.${randomBytes(6).hex}.tmp` so the catch-path unlink only removes the caller's own tmp. Final-file atomicity (P18) preserved, single-writer behavior unchanged, no secret logged, zeroization untouched. Tests (`wallet-kdf-versions.spec`): fchmodSync(0o600) + real stat-mode assertion; unique-tmp regex + interleaved-writer no-cross-delete; L5 covers the `changeWalletPassword` atomic call-site. `atomicWriteFileSync` + `changeWalletPassword` fully covered; 33 wallet tests green.

## [2026-05-30] chore(deps): pin direct dependency versions to lockfile-resolved exact versions (665471fc)

Removed `^`/`~` range specifiers from this package's direct dependencies, pinning each to the EXACT version already resolved in `pnpm-lock.yaml`. Zero behavior change: only the importer `specifier:` lines changed; no resolved `version:`/`resolution:` moved (verified non-specifier-changed=0). Improves supply-chain reproducibility (P22). A subset of deps was intentionally left as ranges where this package's standalone lockfile resolved a newer version than the root workspace lockfile (a pre-existing root-vs-per-package lockfile divergence, tracked separately as an audit finding, not a clean pin).

## [2026-05-30] feat(p2p): register KG_SHARD_SNAPSHOT server handler with coord-attested auth (e63554c6)

Workstream F2. Closes audit `nodeLOW-kg-shard-snapshot-handler-unregistered`: there was no server-side handler for `KG_SHARD_SNAPSHOT_PROTOCOL` (node-runtime only registered `KG_SHARD_QUERY`), so every peer-first snapshot pull failed and fell through to coord. The KNOWN-GAP (`kg-shard-snapshot.ts:17-38`) was blocked on a trusted libp2p-peerId<->app-identity binding + a requester-authorization source; both are solved by the coord-signed peer-identity attestation (`audits/F-identity-binding-design.md`). New `kg-shard-snapshot-server.ts` runs an 8-step fail-closed chain (every reject hard-returns before serving): shape -> `verifyCoordinatorEnvelope` on the attestation (`DOMAIN_PEER_IDENTITY_ATTESTATION`, 86400s, hardcoded coord trust anchor, verifier UNMODIFIED) -> `attestation.body.p2pPeerId === connection.remotePeer.toString()` (binds the attestation to this Noise-authenticated connection) -> `verified === true` -> Ed25519-verify `req|<shardId>|<publishedAtMs>` against `attestation.body.appPubkey` (length-checked; binds the request to the attested identity) -> `publishedAtMs` freshness +/-5min + bounded `ReplayGuard` on the hex sig -> authorize (any verified node -> any shard) -> stream via stream-codec, disk-only (no DB). Generic-detail `SnapshotError` on reject (P17). Wiring: `DOMAIN_PEER_IDENTITY_ATTESTATION` constant (byte-identical shared-vector hex with coord), `HeartbeatResponse.identityAttestation` cached in `HeartbeatHelper` + getter, `SnapshotRequest.attestation` + dialer embeds it (throws before dial if absent), `node-runtime` registers the protocol via `p2pNode.handleProtocol` with a per-handler `ReplayGuard(5min)`. Tests: shared cross-package vector (492-char hex byte-identical to coord) + server-handler spec (3 construction guards, happy path, a reject test for each of steps 1-6 asserting no leak). `superpowers:code-reviewer` SHIP-AS-IS. Node suite 207 passed, 0 failed, 43 skipped.

## [2026-05-30] feat(wallet): transparent weak-PBKDF2 re-encrypt on unlock + atomic keystore write (d6781c85)

Workstream E of the tests-green + follow-ups batch. Legacy v1 keystores (100k PBKDF2 iterations) still decrypted but were never re-encrypted to v2 (600k); only brand-new writes were v2. `loadWallet`, after a SUCCESSFUL decrypt, now re-encrypts a below-baseline keystore (`recordedIterations < PBKDF2_ITERATIONS_V2`) under the SAME password to v2 and atomically persists it over `wallet.json`. BEST-EFFORT (P-crypto/P-secrets): the upgrade is wrapped in try/catch and NEVER throws out of the unlock path; a failure is `logger.warn`-logged (`err.message` only, no secrets) and the already-unlocked wallet is returned; success logs only iteration counts + version. New `atomicWriteFileSync` (P18): write to `<file>.tmp` (`0o600`) in the same directory, loop `writeSync` until the whole buffer is flushed (a short write would otherwise commit a truncated keystore), `fsync`, `close`, `rename` over the original; on any failure unlink the partial tmp and rethrow so the good `wallet.json` is never clobbered. `changeWalletPassword` switched from the non-atomic `writeFileSync` to this helper (dropped the now-unused import). Zeroization unchanged (`decryptWallet` still zeroes `key`/`decrypted`); v2 KDF params come from `encryptWallet`. `staking-cli.ts` uses a separate load path (raw `EncryptedKeystore`/scrypt) and is tracked as a scoped sibling follow-up. Tests: v1->v2 rewrite + same-password round-trip + identical plaintext key; v2 not rewritten; rename/fsync/open failure preserves the byte-identical good original (still decrypts); best-effort failure does not throw; no secret logged. `superpowers:code-reviewer` SHIP-AS-IS (folded its one LOW: the `writeSync` short-write loop). 29 wallet tests green.

## [2026-05-30] fix(cli): block downgrade-install in self-updater runCycle (2b89aa7e)

Workstream D of the tests-green + follow-ups batch. The self-updater had only a passive cross-lifetime staleness guard (`restartedButStillStale`); nothing prevented `runCycle()` from STARTING an install of an OLDER `result.latestVersion` than the currently-running version, so a registry serving a rolled-back/old build could trigger a downgrade-install (P2 version-handling, P22 supply-chain). Added a pre-install guard after the UP_TO_DATE early-return and before the attempt-cap/idle-gate: `if (valid(latest) && lt(latest, running)) { logger.warn(...); return; }`. Coercion mirrors `restartedButStillStale` (`valid()` + `0.0.0` fallback) so prerelease/dirty versions compare consistently and an uncoercible `latest` never blocks a real upgrade. Passive guard untouched. Spec adds: older `latestVersion` blocks (no `selfUpdate`/`restart`/`setDraining`, warn logged); newer proceeds (regression guard). 25 update-manager tests + 115 `src/utils` tests green.

## [2026-05-30] test(langgraph): align BackpressureService mocks with per-class API (7fc788ac)

Workstream A1 of the tests-green + follow-ups batch. The source migrated `BackpressureService` to a per-class API (`isDraining` / `getInFlightByClass` / `getMaxByClass`, `backpressure.service.ts:271/254/285`), consumed by `fetch-work-orders.ts:100`. Three langgraph suites still mocked only the old class-blind API, so they threw `this.backpressure.isDraining is not a function` (3 suites, 11 test failures, single cause). Added the three missing methods to each stale mock (`langgraph-nodes.test.ts` shared `mockBackpressure`, `langgraph-coverage2.test.ts` inline mock, `integration.spec.ts` inline stub), matching the already-correct sibling mock in `nodes/__tests__/fetch-work-orders-drain.spec.ts` (`getMaxByClass`: `HEAVY -> 1`, otherwise `-> 2`). Test-only; no source touched. Node suite now fully green: 205 passed, 0 failed, 43 skipped (expected).

## [2026-05-29] fix(auth): bind HTTP method into node->coord request signature (signer) (a7d3749d)

`buildAuthHeaders` (the single node->coord signer that model-discovery, review-agent, commit-reveal, work-order.coordinator, inference-server and telemetry all route through) now signs `${peerId}:${timestamp}:${METHOD}:${path}:${bodyHash}` (METHOD = `String(method).toUpperCase()`); the verb is now bound into the signature (`method` was already a param, just never in the signed message). Lockstep with the coord `NodeSignatureGuard` verifier; the WS handshake signer is unchanged (no HTTP method). Corrected stale docblocks describing the old format (`inference-server.ts`, `diloco_aggregation_runner.ts` x2) to include METHOD (P10). Specs: `node-auth.spec` gains the shared cross-package vector (byte-identical to the coord assertion) + uppercase normalization; `inference-server.spec` signs the real POST; the diloco aggregation/validation runner parity specs + the work-order GET / gradients-POST parity specs reconstruct the method-bound message with the correct verb. Both reviewers SHIP_AS_IS; full node suite green modulo known pre-existing langgraph backpressure fails. Resolves the deferred cross-side HTTP-auth method-binding follow-up.

## [2026-05-29] fix(p2p): verify domain-separated gossip envelopes + bounded replay guard (0091548f)

Node side of the coordinator gossip domain-separation change (lockstep with the coord `signedEnvelopeBytes` unification; safe as a clean cut-over now that the network is a single operator node deployed together with coord). New `verify-coordinator-envelope.ts` is a single shared verify helper that reconstructs the byte-identical `JSON.stringify({ domain, body, ts })`, verifies the coordinator Ed25519 signature, applies the freshness window, and (when given a replay guard) rejects re-injected signatures; the 3 topic handlers (`work-order-available` body=wo, `work-order-assigned` + `evaluation-assignments` body=payload) now call this one helper with their own domain constant (byte-identical to the coordinator's exported values), removing the 3 duplicated reconstructions and closing the cross-type signature-replay vuln. New `replay-guard.ts` is a bounded `Map<sigHashBase64, expiryEpochMs>` (TTL = freshness window, opportunistic prune past a threshold) so a captured envelope cannot be re-injected within the 60s window; opt-in via an optional `replayGuard?` handler arg, and node-runtime wires one bounded `ReplayGuard(60)` per topic at the gossip dispatch site. Tests: verifier specs updated to the domain-tagged bytes; a verify-helper spec with the shared cross-package vector hex (matches the coord assertion) + wrong-domain rejection; a replay-guard spec; two node-runtime wiring specs. Existing freshness / pubkey-trust / Slice-2 targetPeerId-filter tests preserved. Full node suite green modulo the known pre-existing langgraph backpressure fails.

## [2026-05-29] test(diloco): torch-pin compat harness + corrected version policy (c0922fe1)

Resolves deferred audit item WS3 (DiLoCo torch/numpy pins, validate + document). Corrected `requirements-training.txt` (P10): the re-validation target on a torch bump is SCALAR-INVARIANT stability (`avgGradientNorm` / `velocityNorm` / accept-reject sets per design section 4.2), NOT an adapter sha256 (the serialized adapter bytes are non-deterministic because torch embeds storage metadata, so the adapter hash is informational only); the coordinator verifies each submission blob's sha256 + Ed25519 signature for integrity/auth and never deserializes tensors (no coord-side torch pin), while every training+aggregating node must share the `==` pin so invariants are computed identically. Added `scripts/diloco_compat_smoke.py` (new pre-release harness: runs the real `diloco_aggregate_executor.aggregate` + `diloco_train.compress_gradients_svd` twice with seeded inputs and asserts the scalar invariants are bit-equal, exits non-zero on drift, stays off the transformers path) wired as the `test:diloco-compat` script (prefers the pinned `/opt/training-venv` python), and `docs/adr-0001-diloco-torch-pin-policy.md` documenting the policy + bump procedure. Verified: harness PASS + `diloco_aggregate_executor_test` 19/19 on the locally-installed torch 2.8.0 (the consensus-key path is version-robust; the authoritative pre-release run must use the pinned 2.5.1 venv in CI).
...
## [2026-05-29] fix(node): audit LOW/INFO sweep batches A-D (c8f4d2ba)

Deduplicated LOW/INFO remediation from the 2026-05-29 full-stack audit (node half of the 45-finding file-disjoint sweep). Reviewed per-batch by `superpowers:code-reviewer` (SHIP_AS_IS); full node suite green modulo the known pre-existing langgraph backpressure fails; no new regressions. Highlights: bootstrap import-catch coerces non-Error throws; `node-auth` docblock corrected (identity.json stores raw 32-byte hex, not DER, a P10 false comment) with HTTP/WS key-handling symmetry and the signed-message format UNCHANGED; `bigint-warning-filter` captures the real unpatched `stderr.write` at load; `generate_embedding` / KG / docking-vina params bounds-checked and `checkOllama` tolerates a non-array body; decrypted secret + KDF keys zeroized after use; legacy `<600k`-PBKDF2 wallet load emits a loud security warning; peer-controlled inference-handler input and the unwired react/self-critique prompt builders now route through `assertSafeForPrompt` (P26) and ChatStreamHandler no longer renders remote `system`-role turns; `identity.json` written `0600`; A2A nonce recorded only AFTER signature verification; `KG_SHARD_QUERY` gains k/embedding caps + a bounded per-peer rate limiter and `KG_EMBEDDING_DELTA` enforces `MAX_BATCH=50`; TOPICS source-of-truth consolidated and `publish()` only swallows the precise transient stream-closed condition; `runtime.stop()` tears down the inference HTTP server + RoundListener WS instead of leaking them; real-crypto IdentityHelper spec + de-vacuumed `@libp2p/crypto/keys` / `@noble/hashes` / p2p ed25519 mocks + trust-anchor pinned-pubkey wiring test + downgrade-refusal assertion. PARTIALs (safe subset shipped, follow-ups in-code): weak-PBKDF2 transparent re-encrypt, KG_SHARD_SNAPSHOT verifying handler (needs a requester-auth model, P2), self-updater source-side `lt()` downgrade guard. DEFERRED: node-side gossip replay-protection (coupled to the coord gossip domain-separation signed-byte change, P46).
...
Version-only bump. Node has no functional change in this cycle.
Node-ui 0.8.40 makes Tauri spawn `dist/bootstrap.js` instead of
`dist/index.js` so the bigint-buffer warning filter installed
there actually runs. Lockstep keeps coord + node + node-ui
versioned together.
