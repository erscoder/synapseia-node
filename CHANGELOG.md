...
error `{}` from `getProgramAccounts` followed by `fetch failed`
on the tx. Dashboard staking + claim worked fine — root cause
was the node CLI requiring `STAKING_PROGRAM_ID` and
`SYN_TOKEN_MINT` env vars without fallback while every other
client (dashboard, coord, `chain-info-lightweight`) shipped
canonical devnet defaults.

Mirrored the canonical devnet program IDs from
`packages/dashboard/.env.local` and
`packages/node/src/cli/chain-info-lightweight.ts:315`:
- `STAKING_PROGRAM_ID = CYW5Cprp5JuzaXtPyV8LPBgPzbze6QHnc3oFBAVaFkfw`
- `SYN_TOKEN_MINT = 8iFr3ciQuNeU4vkzQTp7NcWNgRr7AVhwyizNCAszaEQq`

Env vars still win when set, so operators running against a
different cluster (alpha, mainnet flip, …) override as before.

## [2026-05-15] fix(wallet-verify): validate against keystore first, legacy fallback (f9544ba0)

`node-ui` spawns `syn wallet-verify` via Tauri `unlock_wallet` to
validate the user-typed password. The subcommand was hard-coded
to `walletService.load(wallet.json)`, so operators who migrated
their wallet to the `EncryptedKeystore` via `syn start` still saw
the legacy password prompt in the desktop app — typing the new
vault passphrase failed because `wallet-verify` only checked
`wallet.json`.

Subcommand now:
- Resolves the passphrase via `SYNAPSEIA_KEYSTORE_PASSPHRASE_FILE`
  > `SYNAPSEIA_WALLET_PASSWORD` / `WALLET_PASSWORD` (back-compat
  with the env Tauri passes).
- Decrypts the keystore at `$SYNAPSEIA_HOME/wallet.keystore.json`
  when it exists. Emits the same `__WALLET_OK__ <pubkey>` marker
  that Tauri Rust scans for, so the `unlock_wallet` contract is
  unchanged.
- Falls back to legacy `wallet.json` with the same passphrase
  when the keystore is absent.
- Cascade on keystore `INVALID_PASSPHRASE` when legacy ALSO
  exists: retry against legacy with the same passphrase (covers
  operators mid-migration who type their old legacy password).
- Exit codes preserved: 0 ok, 1 INVALID_PASSWORD, 2 missing env,
  3 WALLET_NOT_FOUND, 4 WALLET_LOAD_ERROR.

Reviewer (`superpowers:code-reviewer`) flagged `wallet-create`
as a follow-up MEDIUM — desktop first-time setup still writes a
legacy `wallet.json` that then needs migration. Tracked.
